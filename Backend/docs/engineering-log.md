...
## 16. Writing the tests found a real hostname-validation bug

`POST /api/products` was meant to accept only `noon.com` links, and checked that with `uri.Host.EndsWith("noon.com")`. While writing the integration tests I added a lookalike-host case (`https://evilnoon.com/...`) expecting a `400` — and it would have been accepted, since `evilnoon.com` also ends with `noon.com`. The consequence wasn't dramatic (the crawler would just fail to scrape a non-Noon page), but the endpoint's stated contract was wrong, and every accepted submission spends a GitHub Actions dispatch. **Fix:** accept the host only if it equals `noon.com` or ends with `.noon.com`, with the lookalike as a permanent test case.

Two other things from that pass worth recording. First, a quick mutation check (changing the fake-discount `SpikeThreshold` from 1.1 to 1.5) was caught by exactly the one boundary test that should catch it, which is the evidence the boundary tests actually bite rather than passing vacuously. Second, the API integration tests run on EF Core's in-memory provider, which is more lenient than Npgsql — so the paged/sorted/searched list query was also run by hand against the real Postgres database (search `%` matches a literal percent sign rather than acting as a wildcard; nulls sort last in both directions; the total and page counts match). One slip in that manual pass is worth owning: I probed the crawler with a `--help` flag it doesn't have, which simply started a real crawl and wrote 115 genuine price snapshots (plus 7 flags from the normal detection rule, no notifications) before I killed it. The data is real and consistent with what a scheduled run produces, so it was left in place.

## 17. Bounding an unbounded list, and rate limiting behind a proxy of unknown depth

`GET /api/products` returned every tracked product and the browser did the searching and sorting. It worked at a few dozen rows and had an unbounded payload by design. It's now paged, searched, and sorted in the database, with the headline counts on their own `/stats` endpoint. Two details that mattered: products with no value for the sorted field sort last in *both* directions (Postgres's default NULL placement would otherwise flip them to the top when sorting descending), and ties break on `Id`, because paging over a non-total order can repeat or skip rows between pages.

Rate limiting was added at the same time, and the interesting part is what it can't rely on. Behind the hosting platform's proxies the socket peer isn't the client, so client IPs come from `X-Forwarded-For` — but the number of proxy hops isn't known, and the header is client-forgeable, so a per-IP limit alone is trivially rotated around. The endpoints that matter most are the two that dispatch GitHub Actions runs (a metered resource), so they carry three layers: a global per-IP limit, a tighter per-IP dispatch limit, and a *combined* dispatch cap shared by all callers that holds regardless of what IP anyone claims. The limiter is in-process, so it would not hold across multiple API instances — acceptable for a single free-tier container, and noted as a limitation rather than hidden.

## 18. Testing the scrapers without the live site

The scrapers were the one part of the project verified only by manual runs — and the part most likely to break, since they're welded to Noon's markup. They can't be tested against the live site in CI (anti-bot protection, network, and the answer changes daily), and mocking `IPage` would test the mock rather than the selectors.

**Approach:** run the real scrapers in a real headless Chrome, with the network cut off. Playwright's `RouteAsync("**/*", …)` answers every request the page makes with HTML the test supplies, so `page.GotoAsync("https://www.noon.com/...")` inside the scraper receives saved markup and every selector runs for real. I captured fresh markup from the live site (category tiles, `Product` JSON-LD, the price element, and the "other sellers" panel), trimmed it to the structure the scrapers read, and saved it under `NoonScraper.Tests/Fixtures/` with a README that separates what is a real capture from what is derived (the search-grid tile template, badge text variants, JSON-LD edge cases, and the click-to-open behaviour of the sellers panel).

A few things that fell out of it. The live Mobiles page only used the *carousel* tile template, so `CategoryScraper`'s handling of the search-grid template — which the code comments describe — had never been seen in a capture; the test builds it from a real tile with the two `data-qa` names swapped. The product I'd used to develop the offers feature (an Anker charger with three sellers) now has a single seller, so I had to find another product with a live panel (a Samsung phone with twelve) — a reminder that the fixtures are snapshots of a moving target. And the panel fixture has to inject its cards on click, because the real page doesn't render them until the trigger is pressed.

To check the tests weren't vacuous I broke three things on purpose — the discount selector, the seller-price selector, and `wasPrice > price` → `>=` — and each was caught by the tests that should catch it. What these tests can't do is notice Noon changing its markup; they're a regression check for the scrapers, and a fresh capture is the first step when a production scrape starts failing.

## 19. Moving the tests onto real PostgreSQL found problems the in-memory fake was hiding

The API tests ran on EF Core's in-memory provider, which I'd already noted was more lenient than Npgsql. For a project whose main claims are about *constraints under concurrency* (one row per URL, one active request per product, one restock per snapshot), that isn't a footnote — the in-memory provider doesn't enforce unique or partial indexes, foreign keys, advisory locks or real transactions, so none of those claims could have been tested at all. Every test that touches the database now runs on real PostgreSQL: an embedded server locally (downloaded once, started on a free port), a service container in CI (`NOON_TEST_POSTGRES`). Migrations are applied once to a template database and each test gets a clone of it, so tests stay isolated and parallel.

Doing that found three problems, two of them in the test harness itself, which is worth owning because they made concurrency tests pass or fail for the wrong reasons:

- **The harness created a separate database per concurrent request.** `ApiFactory` built its test database lazily on first access, and with a dozen simultaneous requests the unsynchronized `??=` ran a dozen times — twelve requests, twelve empty databases, twelve `201 Created`s and one row in the database the assertion looked at. The "concurrent submissions create one product" test reported exactly that (`Expected: 1, Actual: 12`), and it took logging every SQL command to see that all twelve `INSERT`s had succeeded *in different databases*. It's now a thread-safe `Lazy`. Any concurrency test that had "passed" before that fix hadn't tested concurrency.
- **The embedded server hung under a full run.** Roughly 20 tests — always the ones that provoke a unique or foreign-key violation on purpose — timed out after 30 seconds, and only in the full suite; each passed alone. Postgres logs every error, its stderr was a pipe nothing was reading, and once enough had accumulated the buffer filled and the next logging statement blocked. Turning the server's error logging off (`log_min_messages=panic`) fixed it, along with `fsync=off` (throwaway data) and capping xUnit's parallelism. The full non-browser run went from ~8 minutes with 19 failures to under 6 minutes; after the harness fix above, everything passes.
- **The tests found real bugs in code I'd already reviewed** — see #22.

Also worth recording: the tests blank the Telegram and GitHub secrets. One test failed only on my machine because `dotnet user-secrets` had a real webhook secret set, and the app (correctly) loads user secrets in Development. A test suite that behaves differently depending on what secrets are on a developer's laptop is one that could, in the wrong setup, use them.

## 20. GitHub Actions as an unreliable trigger, and what that made me build

The original pipeline treated a successful `repository_dispatch` as a promise: insert a `Pending` row, fire the event, poll until something changed it. Written down like that, the gaps are obvious. If the dispatch fails after the row is written, the row is `Pending` forever. If the workflow never starts, same. If it starts twice, two scrapes. If it dies after starting, the row stays `Pending` (there was no `Running`). If the user double-clicks, two runs.

I can't get a queue's guarantees from `repository_dispatch` (nor buy them — see [architecture.md](architecture.md)), so I stopped assuming them and made the *database* the source of truth: the request row is written first; a partial unique index allows one active request per product (the second caller gets the first's request back); the worker claims the row with a single conditional `UPDATE`, so a duplicate delivery does nothing; `Running` is a real state; completion is conditional on still being `Running`, so a late result can't contradict an expiry; and anything not picked up or finished within 15 minutes is expired to `Failed` — by the API on the next request, by the daily crawl, and at read time, so a `GET` can never say "running" for something that isn't. Every failure has a stage and a public-safe message; the GitHub run id and a correlation id are stored on the row and stamped on every log line and on the run's name. The workflows close their own request in a final `failure() || cancelled()` step if they die before the crawler can.

What I chose *not* to build: automatic redelivery of a lost dispatch. That's precisely what a real queue is for, and pretending otherwise with a polling retry loop would be a worse queue. The cost is written down in [known-limitations.md](known-limitations.md).

## 21. One reading, one transaction

`ProductUpserter` was a static method: find the product by URL, add it if missing, append a snapshot, run the restock and discount rules, send Telegram messages. Two things were wrong that only matter once two crawls can overlap — which they can, because the scheduled crawl and an on-demand crawl are separate workflow runs, and a timed-out job can be retried while its predecessor is still alive. Both could insert the same product; both could see "out of stock → in stock" and record two restocks; a discount could be judged against history that didn't yet include the other crawl's snapshot; and a failure halfway left a snapshot without its flag. Notifications were sent inside the same loop, holding whatever state the loop held.

`ProductRecorder` does each reading in one transaction that first takes a Postgres advisory lock on the product URL (which also works when the row doesn't exist yet), sends notifications only after commit, and *claims* each alert with a conditional update before sending it so two crawls can't both send it (and a failed send releases the claim instead of advancing the baseline). There's a test where a database trigger makes one product's flag insert fail, followed by three more products: the failed one leaves nothing behind and the others are unaffected. The daily crawl around it is failure-isolating by design: a category or product failing is counted and skipped, progress is committed per product, there's a time budget that defers the oldest-first remainder, a `CrawlRuns` row with a unique index refuses overlapping runs, and the exit code is an explicit policy rather than "0 unless it crashed".

## 22. Tightening URL validation, and why the tests wrote the spec

The submission validator started as "absolute URL, host ends with `noon.com`" (#16 fixed the `evilnoon.com` case) and grew into `NoonUrl`: https only, no userinfo or port, host compared on its punycode form, a product-shaped path, and a canonical form the crawler visits instead of the raw input. I wrote the tests as an adversarial list first, and they found things I wouldn't have thought to check: a path like `/../x/N1/p/` parsed with market `x`; `ExtractSku("/egypt-en/p/")` returned `egypt-en` as a product code; a string containing two URLs separated by a comma got through; zero-width characters (Unicode "format", not whitespace) got through — a way to make two different strings look identical; and, last, `Uri` quietly percent-encodes `<`, `>`, `"` and `|`, so a whitelist applied to the *normalized* path let them through. The whitelist now runs on the path as the user wrote it — and allows letters from any script, because the first version of that fix rejected real Arabic slugs and a test I'd written for them caught it.

## 23. Contracts and tests on the frontend

The frontend had no tests and its API types were a hand-maintained mirror of the backend DTOs, so a renamed field meant `undefined` several components away. No code generator (a schema-first toolchain would be bigger than the app), but two cheap things: the response shapes are `zod` schemas that every response is validated against at the boundary (a mismatch is a named "unexpected response" error, and the types are inferred from the schemas), and the backend has a `ContractTests` class that pins the JSON the real API produces — for crawl states, failed checks, problem details — into fixture files that the frontend's test parses with those same schemas. Change a DTO and the backend test fails until the fixtures are regenerated (a visible diff); regenerate them and a stale frontend schema fails in CI. Building that caught one thing immediately: my seeded "completed check" fixture used camelCase JSON, the API returned `failureStage: "result"` — the API reads what the crawler writes (PascalCase, the .NET default), so the fixture now serializes an `OfferResult` exactly as the crawler does, which also pins that crawler-to-API contract.

The component tests (submission and its failure statuses, polling for a new product, a failed crawl, a failed check, price-history rendering, sort/filter/paging, the stale-response race, the notify link) use the real API client against a stubbed `fetch`, not a mocked client, so they cover how the UI reacts to what actually comes back over HTTP. Two bugs fell out of writing them: `CheckNowPanel` never stopped its polling timer when the component unmounted, and `ProductDetailPage` reported any load failure — an outage, an unreadable response — as "product not found".
